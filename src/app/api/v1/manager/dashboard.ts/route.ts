...
export async function GET(req: NextRequest) {
  try {
    ...
    // Default mock fallbacks with Royal Blue Theme
    const teamMembers = dbTeam.length > 0 ? dbTeam.map((m) => ({
      id: m._id.toString(),
      ...
      { id: '1', name: 'Aanya Sharma', role: 'Product Designer', avatar: 'AS', color: 'bg-blue-100 text-blue-700', status: 'present', perf: 92, birthday: 'Sep 20', dob: '20 Sep' },
      ...
      { id: '3', name: 'Sneha Pillai', role: 'UX Researcher', avatar: 'SP', color: 'bg-indigo-100 text-indigo-700', status: 'present', perf: 94, birthday: 'Nov 12', dob: '12 Nov' },
      ...
      { id: '5', name: 'Priya Joshi', role: 'Talent Acquisition', avatar: 'PJ', color: 'bg-emerald-100 text-emerald-700', status: 'absent', perf: 80, birthday: 'Dec 5', dob: '05 Dec' },
    ...
    const initialApprovals = dbApprovals.length > 0 ? dbApprovals.map((a) => ({
      ...
      color: a.avatarColor || 'bg-blue-100 text-blue-700',
      type: a.requestType || a.type || 'Leave Request',
      ...
      { id: '2', name: 'Sneha Pillai', avatar: 'SP', color: 'bg-indigo-100 text-indigo-700', type: 'WFH Request', desc: 'Sep 13, Friday', urgency: 'medium', status: 'pending' },
      ...
      { id: '4', name: 'Aanya Sharma', avatar: 'AS', color: 'bg-blue-100 text-blue-700', type: 'Task Reassign', desc: 'Sprint 4 → Sprint 5', urgency: 'medium', status: 'pending' },
      { id: '5', name: 'Priya Joshi', avatar: 'PJ', color: 'bg-emerald-100 text-emerald-700', type: 'Leave Request', desc: 'Sep 20 — 1 day', urgency: 'low', status: 'approved' },
    ];

    const teamLeave = dbLeave.length > 0 ? dbLeave.map((l) => ({
      name: l.employeeName || l.name || 'Team Member',
      avatar: l.avatar || 'TM',
      color: l.avatarColor || 'bg-slate-100 text-slate-700',
      type: l.leaveType || l.type || 'Casual Leave',
      from: l.fromDate || l.from || 'Sep 13',
      to: l.toDate || l.to || 'Sep 13',
      status: l.status || 'Approved',
    })) : [
      { name: 'Vikram Singh', avatar: 'VS', color: 'bg-cyan-100 text-cyan-700', type: 'Casual Leave', from: 'Sep 13', to: 'Sep 13', status: 'On Leave' },
      { name: 'Priya Joshi', avatar: 'PJ', color: 'bg-emerald-100 text-emerald-700', type: 'Sick Leave', from: 'Sep 13', to: 'Sep 14', status: 'Approved' },
      { name: 'Rohit Verma', avatar: 'RV', color: 'bg-sky-100 text-sky-700', type: 'Casual Leave', from: 'Sep 15', to: 'Sep 16', status: 'Pending' },
      { name: 'Sneha Pillai', avatar: 'SP', color: 'bg-indigo-100 text-indigo-700', type: 'WFH', from: 'Sep 13', to: 'Sep 13', status: 'Pending' },
    ];
