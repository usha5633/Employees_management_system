...
const panels = [
  {
    ...
    badgeColor: 'bg-blue-500/10 text-blue-400 border border-blue-500/20',
    cta: 'Open Admin Panel',
    ctaStyle: 'bg-blue-600 hover:bg-blue-500 shadow-lg shadow-blue-600/25',
    features: [
      { icon: Users, label: 'All Employees' },
      { icon: CreditCard, label: 'Payroll' },
      { icon: BarChart3, label: 'Reports' },
...
export default function LandingPage() {
  return (
    <div className="relative min-h-screen bg-[#070B14] font-sans antialiased selection:bg-blue-600 selection:text-white overflow-hidden">
      {/* Dynamic Background Effects */}
      <div className="pointer-events-none fixed inset-0 bg-[linear-gradient(to_right,#1e293b15_1px,transparent_1px),linear-gradient(to_bottom,#1e293b15_1px,transparent_1px)] bg-[size:3.5rem_3.5rem]" />
      <div className="pointer-events-none fixed -top-40 -left-40 h-[500px] w-[500px] rounded-full bg-blue-600/15 blur-[120px]" />
      <div className="pointer-events-none fixed -bottom-40 -right-40 h-[500px] w-[500px] rounded-full bg-blue-700/10 blur-[140px]" />

      <div className="relative mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8">
        {/* Header Section */}
        <div className="mb-16 text-center">
          <div className="mb-6 inline-flex items-center gap-2.5 rounded-full border border-blue-500/20 bg-blue-950/40 px-4 py-1.5 backdrop-blur-xl shadow-lg shadow-blue-600/10">
            <Sparkles className="h-4 w-4 text-blue-400" />
            <span className="text-xs font-bold tracking-wide text-blue-300 uppercase">
              Infinite Cloud · Enterprise HRMS
            </span>
          </div>
          ...
          <p className="mx-auto mt-4 max-w-xl text-xs sm:text-sm leading-relaxed text-slate-400 font-medium">
            Unified workforce governance system with real-time operational portals for Management, HR, Team Leads, and Employees.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
          {panels.map((panel) => {
            ...
            return (
              <div
                key={panel.role}
                className="group relative flex flex-col overflow-hidden rounded-3xl border border-slate-800/80 bg-slate-900/40 backdrop-blur-xl transition-all duration-300 hover:-translate-y-1.5 hover:border-blue-500/40 hover:bg-slate-900/80 hover:shadow-2xl hover:shadow-blue-600/10"
              >
                ...
                <div className="flex flex-1 flex-col p-6 sm:p-7">
                  <div className="mb-5 flex items-center justify-between">
                    <div className={`flex h-12 w-12 items-center justify-center rounded-2xl ${panel.iconBg} text-white transition-transform group-hover:scale-105`}>
                  ...
                  <p className="mt-2 text-xs leading-relaxed text-slate-400 min-h-[38px] font-medium">
                    {panel.description}
                  </p>
                  ...
                  <div className="mt-6 grid grid-cols-2 gap-2">
                    {panel.features.map(({ icon: FIcon, label }) => (
                      <div
                        key={label}
                        className="flex items-center gap-2 rounded-xl border border-slate-800/80 bg-slate-800/30 px-3 py-2 transition-colors group-hover:border-slate-700/80"
                      >
                        <FIcon className="h-3.5 w-3.5 shrink-0 text-blue-400" />
                  ...
                  <Link
                    href={panel.href}
                    className={`mt-7 flex w-full items-center justify-center gap-2 rounded-xl px-4 py-3 text-xs font-bold text-white transition-all active:scale-[0.98] ${panel.ctaStyle}`}
                  >
                    {panel.cta}
                    <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
                  </Link>
        ...
        {/* Unified Royal Blue Quick Direct Links */}
        <div className="mt-12 rounded-3xl border border-blue-900/30 bg-slate-900/30 p-6 backdrop-blur-xl shadow-2xl">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {quickLinks.map(({ label, href }) => (
              <Link
                key={label}
                href={href}
                className="flex items-center gap-1.5 rounded-xl border border-blue-900/50 bg-blue-950/20 px-3.5 py-2 text-xs font-bold text-blue-400 hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all shadow-sm"
              >
        ...
        {/* Footer info */}
        <p className="mt-10 text-center text-[11px] font-semibold tracking-wide text-slate-600">
          Infinite Cloud EMS · Enterprise 4-Panel HRMS Infrastructure v2.0
        </p>
      </div>
    </div>
  );
