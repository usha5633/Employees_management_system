...
import {
  ...
  ChevronDown, ShieldCheck, LogOut, Settings, User, Sparkles,
} from 'lucide-react';
...
export default function HRPageShell({ 
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-[#F8FAFC] font-sans text-slate-900 antialiased selection:bg-emerald-600 selection:text-white">
      ...
      <aside className={`fixed top-0 bottom-0 left-0 z-50 flex w-64 flex-col border-r border-slate-800/80 bg-[#0B1120]/95 text-slate-200 backdrop-blur-2xl transition-transform duration-300 ease-in-out lg:static lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="flex h-16 shrink-0 items-center justify-between border-b border-slate-800/80 px-5">
          <Link href="/hr" className="flex items-center gap-3 group">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-tr from-emerald-700 to-emerald-500 font-black text-white shadow-lg shadow-emerald-600/30 transition-transform group-hover:scale-105">
              <span className="text-xs font-black tracking-wider">HR</span>
            </div>
            <div>
              <div className="flex items-center gap-1.5">
                <span className="text-base font-black tracking-tight text-white">HR Panel</span>
                <span className="text-[10px] font-extrabold uppercase tracking-widest text-emerald-400">OPS</span>
              </div>
              <p className="text-[10px] font-medium text-slate-400">People Operations</p>
            </div>
          </Link>
          <button 
            onClick={() => setIsSidebarOpen(false)} 
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Icon className={`h-4 w-4 shrink-0 transition-colors ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-emerald-400'}`} />
                        <span>{label}</span>
                      </div>
                      {badge && (
        ...
        <div className="shrink-0 border-t border-slate-800/80 p-3.5">
          <div className="mb-2 flex items-center gap-3 rounded-xl border border-emerald-900/30 bg-emerald-950/20 p-2.5 backdrop-blur-md">
            <div className="relative flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-emerald-600 text-xs font-extrabold text-white shadow-sm">
              HM
              <span className="absolute bottom-0 right-0 h-2 w-2 rounded-full bg-emerald-400 ring-2 ring-slate-900" />
            </div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-xs font-bold text-slate-200">HR Manager</p>
              <p className="truncate text-[10px] font-medium text-slate-400">hr@company.com</p>
            </div>
          </div>
          <Link 
            href="/login"
        ...
        <header className="sticky top-0 z-30 flex h-16 w-full shrink-0 items-center justify-between border-b border-slate-200/80 bg-white/80 px-4 backdrop-blur-xl sm:px-6">
          ...
          <div className="hidden max-w-sm flex-1 px-6 md:block">
            <div className="relative">
              ...
              <input 
                type="text" 
                placeholder="Search employees, documents, leave applications..." 
                className="w-full rounded-xl border border-slate-200/80 bg-slate-50/80 py-2 pl-10 pr-4 text-xs font-medium text-slate-800 placeholder-slate-400 outline-none transition-all focus:border-emerald-600 focus:bg-white focus:ring-2 focus:ring-emerald-600/10" 
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
              ...
              {isNotificationsOpen && (
                <div className="absolute right-0 mt-2 w-80 rounded-2xl border border-slate-200/80 bg-white/95 p-3 shadow-2xl backdrop-blur-xl z-50 animate-fade-in">
                  <div className="flex items-center justify-between border-b border-slate-100 pb-2.5 mb-2 px-1">
                    ...
                    <span className="rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-bold text-emerald-600 border border-emerald-100">4 New</span>
                  </div>
                  <div className="space-y-2 max-h-60 overflow-y-auto">
                    {[
                      ...
                      <div key={n.t} className="rounded-xl bg-slate-50 p-2.5 text-xs hover:bg-slate-100/80 transition-colors">
                        <p className="font-bold text-[#0D1222]">{n.t}</p>
                        <p className="text-slate-500 text-[11px] mt-0.5">{n.b}</p>
                        <span className="mt-1 block text-[9px] font-semibold text-slate-400">{n.time}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="relative" ref={profileRef}>
              ...
              >
                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-emerald-600 text-xs font-black text-white shadow-md shadow-emerald-600/20">
                  HM
                </div>
                <div className="hidden text-left sm:block">
                  <div className="flex items-center gap-1">
                    <span className="text-xs font-bold text-[#0D1222]">HR Manager</span>
                    <ShieldCheck className="h-3.5 w-3.5 text-emerald-600" />
                  </div>
                  <p className="text-[10px] font-medium text-slate-400">HR Operations</p>
                </div>
              ...
              {isProfileOpen && (
                <div className="absolute right-0 mt-2 w-56 rounded-2xl border border-slate-200/80 bg-white/95 p-2 shadow-2xl backdrop-blur-xl z-50 animate-fade-in">
                  <div className="border-b border-slate-100 px-3 py-2 mb-1">
                    <p className="text-xs font-bold text-[#0D1222]">HR Manager</p>
                    <p className="text-[11px] font-medium text-slate-400 truncate">hr@company.com</p>
                  </div>
                  <div className="space-y-0.5">
                    <Link 
                      href="/hr/settings" 
                      onClick={() => setIsProfileOpen(false)} 
                      className="flex items-center gap-2.5 rounded-xl px-3 py-2 text-xs font-bold text-slate-700 hover:bg-emerald-50 hover:text-emerald-600 transition-colors"
                    >
