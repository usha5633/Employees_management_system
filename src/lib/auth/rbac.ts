...
/**
 * LOGIN DISABLED (temporary): when there is no session cookie, instead of
 * requiring sign-in we fall back to the seeded admin account so every panel
 * is reachable directly. Remove this block (and require a real session
 * again) whenever login should be turned back on.
 */
const LOGIN_DISABLED = true;

async function getFallbackAdminContext(): Promise<AuthContext> {
  const db = await getDb();
  const user = await db.collection(COLLECTIONS.USERS).findOne({ role: 'admin' });

  if (!user) {
    throw new AuthError('No admin account found to bypass login with. Run "npm run seed" first.', 401);
  }
 ...
 * Resolves the authenticated user + their effective permissions for the
 * current request, based on the `ems_session` cookie. Throws AuthError
 * (401) if there is no valid session (unless LOGIN_DISABLED is true, in
